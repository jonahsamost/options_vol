...
class call_butterfly:
    ...
    def _add(self):
        ...
        if not ls: return 
        if ls == bs.LONG: legs = [bs.LONG,bs.SHORT,bs.LONG]
        else: legs = [bs.SHORT,bs.LONG,bs.SHORT]
        cnt = 0 

        l1 = self.leg1.currentText() 
        l2 = self.leg2.currentText() 
        l3 = self.leg3.currentText() 
        if l1==self.place or l2==self.place or l3==self.place: return 
    ...
    def _date_chosen(self,desc):
        ...
        self.leg3.addItems([self.place] + names) 
    ...
    def _callback(self):
        ...
        strat = QFormLayout()
        strat.addRow('1x', self.leg1)
        strat.addRow('2x', self.leg2)
        strat.addRow('1x', self.leg3)
        strat.addRow('', go)
