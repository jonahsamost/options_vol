...
class strip:
    ...
    def _add(self):
        ...
        if not ls: return 
        if ls == bs.LONG: legs = [bs.LONG,bs.LONG,bs.LONG]
        else: legs = [bs.SHORT,bs.SHORT,bs.SHORT]
        cnt = 0 
    ...
    def _date_chosen(self,desc):
        ...
        self.leg1.clear() 
        self.leg2.clear() 
        self.leg1.addItems([self.place] + calls) 
    ...
    def _callback(self):
        ...
        strat = QFormLayout()
        strat.addRow('Opt (1x)', self.leg1)
        strat.addRow('Opt (2x)', self.leg2)
        strat.addRow('', go)
